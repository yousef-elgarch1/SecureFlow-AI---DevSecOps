...
import { Upload, Activity, CheckCircle, Github, Code2 } from 'lucide-react';
...
function App() {
  const [inputMode, setInputMode] = useState('upload'); // 'upload' or 'github'
  ...
  const handleGeneratePolicies = async () => {
    ...
    try {
      ...
      if (inputMode === 'upload') {
        result = await apiClient.generatePolicies(files, 5, handleProgress);
      } else if (inputMode === 'github') {
